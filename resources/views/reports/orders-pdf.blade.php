...
<body>
    ...
    <div class="summary">
        ...
        <div class="summary-item">
            <h4>Items Total</h4>
            <p>{{ $orders->sum(function($order) { return $order->orderDetails->sum('quantity'); }) }}</p>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ Str::limit($order->observations ?? 'Sin observaciones', 30) }}</td>
            </tr>
            @endforeach
            <tr class="total-row">
                ...
                <td><strong>S/. {{ number_format($orders->sum('discount'), 2) }}</strong></td>
